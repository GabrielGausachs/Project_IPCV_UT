...
def track_points(prev_frame, current_frame, points):
    ...
    current_points, status, _ = cv2.calcOpticalFlowPyrLK(
        prev_frame, current_frame, prev_points, None
    )
...
def overlay_advertisement(frame, ad_image, homography_matrix):
    ...
    ad_points_video = cv2.perspectiveTransform(
        np.array([ad_points_real_world]), homography_matrix
    )[0]
...
def detecting_lines_intersection_points(frame, prev_frame, ad_image):
    global tracking_points, H, tracking_gp_points, P
    ...
    if tracking_points is None:
        ...
        centroids = cluster_intersection_points(
            intersection_points, eps=50, min_samples=1
        )
        goal_line_points = filter_goal_line_points(
            centroids, max_x_dist=300, max_y_dist=50
        )
    ...
    if (
        ...
        and tracking_gp_points is not None
    ):
        ...
        if tracked_gp_points:
            ...
            P = compute_projection_matrix(tracking_gp_points, goal_post_points_3D)
            compute_goal_post_reprojection_error(
                P, goal_post_points_3D, tracking_gp_points
            )
...
def finding_goalpost(frame, homography_matrix):
    ...
    goalpostarea_world_points = np.array(
        [[5.0, 5.5], [14.0, 5.5]],  # First point to mark  # Second point to mark
        dtype="float32",
    )

    goalpost_bottom_world_points = np.array(
        [[5.7, 5.5], [13.1, 5.5]],  # First point to mark  # Second point to mark
        dtype="float32",
    )

    # Convert the real-world points to image coordinates
    video_points = cv2.perspectiveTransform(
        np.array([goalpostarea_world_points], dtype="float32"), homography_matrix
    )[0]
    bottom_goalpost_points = cv2.perspectiveTransform(
        np.array([goalpost_bottom_world_points], dtype="float32"), homography_matrix
    )[0]
    ...
    lines = cv2.HoughLinesP(
        edges, 1, np.pi / 180, threshold=75, minLineLength=20, maxLineGap=10
    )
    ...
    for bottom_point in bottom_goalpost_points:
        ...
        if closest_line:
            adjusted_top_point = (
                closest_line[0] + min_x,
                highest_y + (bottom_y - cropped_frame.shape[0]),
            )
...
def mark_real_world_points(frame, points):
    ...
    for point in points:
        ...
        cv2.circle(
            frame, (x, y), radius=5, color=(0, 0, 255), thickness=-1
        )  # Red circle
...
def project_ad(frame, ad_image, ad_points_3D, P):
    ...
    ad_points_3D_homogeneous = np.hstack(
        (ad_points_3D, np.ones((ad_points_3D.shape[0], 1)))
    )

    # Project the 3D ad points to 2D using P
    ad_points_2D_homogeneous = P @ ad_points_3D_homogeneous.T
    ad_points_2D = (
        ad_points_2D_homogeneous[:2] / ad_points_2D_homogeneous[2]
    ).T  # Convert to 2D
    ...
    ad_corners_src = np.array(
        [[0, 0], [ad_img_width, 0], [0, ad_img_height], [ad_img_width, ad_img_height]],
        dtype="float32",
    )

    # Compute the perspective transform to fit the ad in the scene
    transform_matrix = cv2.getPerspectiveTransform(
        ad_corners_src, ad_points_2D.astype("float32")
    )
    warped_ad = cv2.warpPerspective(
        ad_image, transform_matrix, (frame.shape[1], frame.shape[0])
    )
...
def compute_goal_post_reprojection_error(P, goal_post_points_3D, goal_post_points_2D):
    ...
    goal_post_3D_homogeneous = np.hstack(
        (goal_post_points_3D, np.ones((goal_post_points_3D.shape[0], 1)))
    )

    # Project the goal post 3D points to 2D using P
    goal_post_2D_homogeneous = P @ goal_post_3D_homogeneous.T
    goal_post_2D_projected = (
        goal_post_2D_homogeneous[:2] / goal_post_2D_homogeneous[2]
    ).T  # Convert to 2D
